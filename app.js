...
const days = [
  { date:"未知周期 · 第 18 天", index:"-2.76%", temp:38, mood:"谨慎", title:"市场连续第三天下跌，热门板块出现明显分化", detail:"部分投资者开始减仓，长期资金仍保持克制。你只能看到此刻已经发生的信息。" },
  { date:"未知周期 · 第 19 天", index:"+1.14%", temp:51, mood:"观望", title:"主要指数低开高走，市场情绪有所修复", detail:"昨日下跌较多的板块反弹，但成交没有明显放大。一次反弹并不等于趋势已经改变。" },
  { date:"未知周期 · 第 20 天", index:"+3.48%", temp:72, mood:"乐观", title:"热门主题快速反弹，社交讨论热度升高", detail:"上涨带来新的乐观情绪，也可能让人忽略原本制定的仓位上限。" },
  { date:"未知周期 · 第 21 天", index:"-1.86%", temp:56, mood:"分化", title:"高波动主题冲高回落，宽基相对稳定", detail:"不同基金的表现开始分化。组合中的每只基金会产生不同的当日收益。" },
  { date:"未知周期 · 第 22 天", index:"+0.72%", temp:61, mood:"谨慎乐观", title:"市场小幅修复，成交保持平稳", detail:"市场没有给出明确方向，继续持有也是一种主动决定，但不需要每天重复点击。" }
];
...
const beginnerLessons = [
  ...
  },
  {
    ...
    explain:"场外基金通常按确认规则和当日净值计算，并非像普通商品立即交付。申购、赎回和持有过程中还可能产生费用。",
    example:"下午某个时间点前后提交，可能按不同交易日确认；持有时间不同，赎回费也可能不同。",
    points:["认识交易日与确认日","查看申购和赎回费","不要把短期频繁买卖当成免费"]
  },
  {
    title:"仓位、分散和定投怎么用",
...
function behaviorMetrics() {
  ...
  const score = Math.max(35, Math.min(95, 58 + calm * 3 + reviewed.length * 4 - chase * 5));
...
function renderHome() {
  ...
  $("#home-account-return").className = returnRate >= 0 ? "soft-positive" : "negative";
  $("#home-training-days").textContent = `${dayIndex + 1} / ${days.length} 天`;
  $("#home-skill-score").textContent = metrics.score;
  $("#home-score-note").textContent = entries.length ? `来自 ${entries.length} 次真实决定` : "从第一笔记录开始成长";
...
function renderAccount() {
  ...
  $("#account-profit").className = snapshot.profit >= 0 ? "positive" : "negative";
  ...
  $("#holdings-list").innerHTML = snapshot.holdings.length ? snapshot.holdings.map(item => {
    ...
    return `<article class="holding-row">
      <div class="holding-name"><span class="fund-symbol ${item.fund.tone}">${item.fund.name.slice(0,1)}</span><div><strong>${item.fund.name}</strong><small>成本 ¥${item.holding.cost.toLocaleString("zh-CN", { maximumFractionDigits:0 })}</small></div></div>
      <strong>¥${item.value.toLocaleString("zh-CN", { maximumFractionDigits:0 })}</strong>
      <span class="${todayReturn >= 0 ? "positive" : "negative"}">${todayReturn >= 0 ? "+" : ""}${todayReturn.toFixed(2)}%</span>
      <span class="${profitRate >= 0 ? "positive" : "negative"}">${profitRate >= 0 ? "+" : ""}${profitRate.toFixed(2)}%</span>
...
function renderLessonInteraction(index) {
  ...
  const interactions = [
    ...
    `<span class="interaction-label">小任务 · 买入前最应该先确认什么？</span>
     <div class="interaction-options"><button data-mini-answer="no">今天是不是上涨</button><button data-mini-answer="yes">确认规则和费用</button><button data-mini-answer="no">别人买了多少</button></div>
     <p class="interaction-feedback">交易确认时间和收费方式会直接影响实际结果。</p>`,
...
function renderFunds() {
  ...
  const visible = funds.filter(f => (fundFilter === "全部" || f.category === fundFilter) && `${f.name}${f.code}${f.category}`.toLowerCase().includes(query));
  $("#fund-grid").innerHTML = visible.map(f => {
    const currentReturn = f.returns[dayIndex] || 0;
    return `
    <article class="fund-card glass ${f.tone}">
      <div class="fund-top"><span class="fund-symbol ${f.tone}">${f.name.slice(0,1)}</span><div><span class="tag">${f.category}</span><span class="tag ${f.risk === "高风险" ? "rose" : f.risk === "低风险" ? "mint" : "lilac"}">${f.risk}</span></div></div>
      <h3>${f.name}</h3><span class="fund-code">${f.code}</span><p>${f.desc}</p>
      <div class="lesson-line"><span>本标的训练重点</span><strong>${f.lesson}</strong></div>
      <div class="fund-bottom"><span class="${currentReturn >= 0 ? "positive" : "negative"}">第 ${dayIndex + 1} 天 ${currentReturn >= 0 ? "+" : ""}${currentReturn.toFixed(2)}%</span><button data-train="${f.id}">用它训练 →</button></div>
    </article>`;
  }).join("");
...
$("#training-fund").innerHTML = funds.map(f => `<option value="${f.id}">${f.name}</option>`).join("");
renderFunds();
...
function renderDay() {
  ...
  $("#market-index").className = day.index.startsWith("+") ? "positive" : "negative";
  ...
  $("#next-day").disabled = dayIndex === days.length - 1 || session.status === "completed";
  $("#finish-day").disabled = dayIndex === days.length - 1 || session.status === "completed";
...
function advanceDay() {
  ...
  renderJournal();
  renderReview();
  showToast("持仓已保留，昨天的决定现在可以复盘了");
}

$("#next-day").addEventListener("click", advanceDay);
$("#finish-day").addEventListener("click", advanceDay);
...
function renderSessionSummary() {
  ...
  $("#summary-return").className = returnRate >= 0 ? "positive" : "negative";
  ...
  $("#session-summary-copy").textContent = `你完成了 ${dayIndex + 1} 个交易日、${entries.length} 次决定和 ${reviewed} 次复盘。收益只是结果，日志才解释你如何走到这里。`;
}
...
$("#save-decision").addEventListener("click", () => {
  ...
  if (decision === "买入") {
    ...
    const holding = account.holdings[fundId] || { units:0, cost:0 };
  ...
  $("#journal-list").innerHTML = visibleEntries.length ? visibleEntries.map((e) => {
    ...
    <article class="timeline-entry">
      ...
      <button class="entry-summary" data-entry="${e.id}">
        ...
        <div class="entry-main"><div><span class="tag ${e.action === "买入" ? "mint" : e.action === "卖出" ? "rose" : "lilac"}">${e.action}</span><h3>${e.fund}</h3></div><p>${e.reason}</p></div>
        <div class="entry-result">${e.amount ? `<strong>¥ ${e.amount.toLocaleString()}</strong>` : ""}<span class="${resultReady ? e.result >= 0 ? "positive" : "negative" : ""}">${resultText}</span></div>
        <span class="expand-icon">+</span>
      </button>
      <div class="entry-detail" hidden>
        <div><small>当时情绪</small><strong>${e.mood}</strong></div>
        <div><small>原始理由</small><strong>${e.reason}</strong></div>
        <div><small>${e.review ? "后来复盘" : "复盘状态"}</small><strong>${e.review ? `${e.review.choice}：${e.review.note}` : resultReady ? "还没有回看这次决定" : "进入下一交易日后才能复盘结果"}</strong></div>
...
function renderReviewTask() {
  ...
  $("#review-entry-result").className = target.result >= 0 ? "positive" : "negative";
...
$("#export-journal").addEventListener("click", () => {
  ...
  const rows = [
    ["训练日","操作","基金","金额","理由","情绪","下一交易日变化","复盘选择","复盘记录"],
